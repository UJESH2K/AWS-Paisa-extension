...
> "101 backend tests against mocked AWS, and 142 checks that load the real extension into a real browser and drive it. Including the ones that matter: that Cost Explorer is called exactly once across repeated reads, and that the session token never reaches the page."
